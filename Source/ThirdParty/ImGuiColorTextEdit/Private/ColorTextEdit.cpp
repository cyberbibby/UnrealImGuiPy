...
void ColorTextEdit::InitCodeFont(ImFont* codeFont)
{
	mCodeFont = codeFont;
}

void ColorTextEdit::SetLanguageDefinition(const LanguageDefinition& aLanguageDef)
{
...
const char* ColorTextEdit::GetLanguageDefinitionName() const
{
	return mLanguageDefinition != nullptr ? mLanguageDefinition->mName.c_str() : "unknown";
}

void ColorTextEdit::SetPalette(const Palette& aValue)
{
	mPaletteBase = aValue;
}

std::string ColorTextEdit::GetText(const Coordinates& aStart, const Coordinates& aEnd) const
{
	std::string result;
...
ColorTextEdit::Coordinates ColorTextEdit::GetCorrectCursorPosition()
{
	auto curPos = GetCursorPosition();

	if (curPos.mLine >= 0 && curPos.mLine <= GetCursorPosition().mLine) {
		for (int c = 0; c < std::min<int>(curPos.mLine, (int)mLines[curPos.mLine].size()); c++) {
...
ColorTextEdit::Coordinates ColorTextEdit::GetActualCursorCoordinates(int aCursor) const
{
	if (aCursor == -1)
...
ColorTextEdit::Coordinates ColorTextEdit::SanitizeCoordinates(const Coordinates& aValue) const
{
...
static int UTF8CharLength(ColorTextEdit::Char c)
{
	if ((c & 0xFE) == 0xFC)
		return 6;
	if ((c & 0xFC) == 0xF8)
...
void ColorTextEdit::Advance(Coordinates& aCoordinates) const
{
...
void ColorTextEdit::DeleteRange(const Coordinates& aStart, const Coordinates& aEnd)
{
	assert(aEnd >= aStart);
	assert(!mReadOnly);
...
int ColorTextEdit::InsertTextAt(Coordinates& /* inout */ aWhere, const char* aValue)
{
	assert(!mReadOnly);
...
void ColorTextEdit::AddUndo(UndoRecord& aValue)
{
	assert(!mReadOnly);
...
ColorTextEdit::Coordinates ColorTextEdit::ScreenPosToCoordinates(const ImVec2& aPosition, bool aInsertionMode, bool* isOverLineNumber) const
{
...
ColorTextEdit::Coordinates ColorTextEdit::FindWordStart(const Coordinates& aFrom) const
{
...
ColorTextEdit::Coordinates ColorTextEdit::FindWordEnd(const Coordinates& aFrom) const
{
	Coordinates at = aFrom;
	if (at.mLine >= (int)mLines.size())
		return at;
...
ColorTextEdit::Coordinates ColorTextEdit::FindNextWord(const Coordinates& aFrom) const
{
	Coordinates at = aFrom;
	if (at.mLine >= (int)mLines.size())
		return at;
...
int ColorTextEdit::GetCharacterIndexLeftSide(const Coordinates& aCoordinates) const
{
...
int ColorTextEdit::GetCharacterIndex(const Coordinates& aCoordinates) const
{
	if (aCoordinates.mLine >= mLines.size())
		return -1;
	auto& line = mLines[aCoordinates.mLine];
...
int ColorTextEdit::GetCharacterColumn(int aLine, int aIndex) const
{
...
int ColorTextEdit::GetLineCharacterCount(int aLine) const
{
	if (aLine >= mLines.size())
		return 0;
	auto& line = mLines[aLine];
...
int ColorTextEdit::GetLineMaxColumn(int aLine) const
{
	if (aLine >= mLines.size())
		return 0;
	auto& line = mLines[aLine];
...
bool ColorTextEdit::IsOnWordBoundary(const Coordinates& aAt) const
{
...
void ColorTextEdit::RemoveLines(int aStart, int aEnd)
{
	assert(!mReadOnly);
	assert(aEnd >= aStart);
...
void ColorTextEdit::RemoveLine(int aIndex, const std::unordered_set<int>* aHandledCursors)
{
	assert(!mReadOnly);
...
void ColorTextEdit::RemoveCurrentLines()
{
	UndoRecord u;
	u.mBefore = mState;
...
void ColorTextEdit::OnLineChanged(bool aBeforeChange, int aLine, int aColumn, int aCharCount, bool aDeleted)
{
...
void ColorTextEdit::RemoveGlyphsFromLine(int aLine, int aStartChar, int aEndChar)
{
...
void ColorTextEdit::AddGlyphsToLine(int aLine, int aTargetIndex, Line::iterator aSourceStart, Line::iterator aSourceEnd)
{
...
void ColorTextEdit::AddGlyphToLine(int aLine, int aTargetIndex, Glyph aGlyph)
{
	int targetColumn = GetCharacterColumn(aLine, aTargetIndex);
	auto& line = mLines[aLine];
...
ColorTextEdit::Line& ColorTextEdit::InsertLine(int aIndex)
{
	assert(!mReadOnly);
...
std::string ColorTextEdit::GetWordUnderCursor() const
{
	auto c = GetCursorPosition();
	return GetWordAt(c);
}

std::string ColorTextEdit::GetWordAt(const Coordinates& aCoords) const
{
...
ImU32 ColorTextEdit::GetGlyphColor(const Glyph& aGlyph) const
{
	if (!mColorizerEnabled)
...
bool ColorTextEdit::IsGlyphWordChar(const Glyph& aGlyph)
{
...
void ColorTextEdit::HandleKeyboardInputs(bool aParentIsFocused)
{
	if (ImGui::IsWindowFocused() || aParentIsFocused)
	{
		...
		if (!IsReadOnly() && isShortcut && ImGui::IsKeyPressed(ImGuiKey_Z))
		...
		else if (isCtrlOnly && ImGui::IsKeyReleased(ImGuiKey_PageUp))
			mCtrlPageUpPressed = true; // Hand over the application layer to handle
		else if (isCtrlOnly && ImGui::IsKeyReleased(ImGuiKey_PageDown))
			mCtrlPageDownPressed = true; // Hand over the application layer to handle
			...
			EnterCharacter('\n', false); // CTRL+ENTER == NEWLINE
		else if (!IsReadOnly() && !alt && !ctrl && !shift && !super && (ImGui::IsKeyPressed(ImGuiKey_Enter) || ImGui::IsKeyPressed(ImGuiKey_KeypadEnter)))
			mEntered = true; // Hand over the application layer to handle
...
void ColorTextEdit::HandleMouseInputs()
{
	ImGuiIO& io = ImGui::GetIO();
	auto shift = io.KeyShift;
...
void ColorTextEdit::UpdatePalette()
{
...
void ColorTextEdit::Render(bool aParentIsFocused)
{
	...
	if (!mLines.empty())
	{
		...
		if (mHighlightBrackets &&
			mState.mCursors[0].mSelectionStart == mState.mCursors[0].mSelectionEnd &&
					...
					while (!highlightBrackets && highlightStart.mLine >= 0) {
						...
						if (!highlightBrackets) {
							...
							if (highlightStart.mLine >= 0)
								highlightStart.mColumn = (int)mLines[highlightStart.mLine].size() - 1;
						}
					}
				}
			}
		...
		while (lineNo <= lineMax)
		{
			...
			if (mState.mCursors[0].mCursorPosition.mLine == lineNo && !HasSelection())
			{
			...
			for (int i = 0; i < line.size();)
			{
				...
				if (glyph.mChar == '\t')
				{
					...
					if (mShowWhitespaces)
					{
						...
						drawList->AddLine(p1, p2, mPalette[(int)PaletteIndex::WhiteSpaceTab]);
						drawList->AddLine(p2, p3, mPalette[(int)PaletteIndex::WhiteSpaceTab]);
						drawList->AddLine(p2, p4, mPalette[(int)PaletteIndex::WhiteSpaceTab]);
					}
				}
				else if (glyph.mChar == ' ')
				{
			...
			if (mState.mCursors[0].mCursorPosition.mLine == lineNo && ImGui::IsWindowFocused())
			{
				// Render the cursor
				...
				{
					float width = 1.0f;
					auto cindex = GetCharacterIndex(mState.mCursors[0].mCursorPosition);
					float cx = TextDistanceToLineStart(mState.mCursors[0].mCursorPosition);
		...
		if (ImGui::IsMousePosValid() && ImGui::IsWindowHovered() && mLanguageDefinition != nullptr)
		{
			...
			if (local.x >= mTextStart)
			{
				...
				if (!id.empty())
				{
					...
					{
						auto identifier = mLanguageDefinition->mPreprocIdentifiers.find(id);
						if (identifier != mLanguageDefinition->mPreprocIdentifiers.end())
						{
							ImGui::BeginTooltip();
							ImGui::TextUnformatted(identifier->second.mDeclaration.c_str());
							ImGui::EndTooltip();
						}
					}
				}
...
void ColorTextEdit::Render(const char* aTitle, bool aParentIsFocused, const ImVec2& aSize, bool aBorder)
{
	for (int c = 0; c <= mState.mCurrentCursor; c++)
	{
	...
	ImGui::PushFont(mCodeFont->IsLoaded() ? mCodeFont : nullptr);
...
void ColorTextEdit::SetText(const std::string& aText)
{
	mLines.clear();
...
void ColorTextEdit::SetTextLines(const std::vector<std::string>& aLines)
{
	mLines.clear();
...
void ColorTextEdit::ChangeCurrentLinesIndentation(bool aIncrease)
{
	assert(!mReadOnly);
	...
	for (int c = mState.mCurrentCursor; c > -1; c--)
	{
		...
		for (int i = start.mLine; i <= end.mLine; i++)
		{
			...
			{
				AddGlyphToLine(i, 0, Glyph('\t', ColorTextEdit::PaletteIndex::Background));
				modified = true;
			}
		}
...
void ColorTextEdit::EnterCharacter(ImWchar aChar, bool aShift)
{
	assert(!mReadOnly);
	...
	if (mCompleteBraces) {
		...
		if (aChar == '{' || aChar == '(' || aChar == '[') {
			mState.mCursors[0].mCursorPosition.mColumn--;
			mInsertedBrace = aChar;
		}
	}

	if (insertEmptyLineOnBrace) {
		auto pos = mState.mCursors[0].mCursorPosition;
		EnterCharacter('\n', true);
		mState.mCursors[0].mCursorPosition = pos;
		EnterCharacter('\t', false);
	}
}

void ColorTextEdit::SetReadOnly(bool aValue)
{
	mReadOnly = aValue;
}

void ColorTextEdit::OnCursorPositionChanged(int aCursor)
{
...
void ColorTextEdit::SetColorizerEnable(bool aValue)
{
	mColorizerEnabled = aValue;
}

void ColorTextEdit::SetCursorPosition(const Coordinates& aPosition, int aCursor)
{
	if (aCursor == -1)
...
void ColorTextEdit::SetSelectionStart(const Coordinates& aPosition, int aCursor)
{
	if (aCursor == -1)
		aCursor = mState.mCurrentCursor;
...
void ColorTextEdit::SetSelectionEnd(const Coordinates& aPosition, int aCursor)
{
	if (aCursor == -1)
		aCursor = mState.mCurrentCursor;
...
void ColorTextEdit::SetSelection(const Coordinates& aStart, const Coordinates& aEnd, SelectionMode aMode, int aCursor, bool isSpawningNewCursor)
{
	if (aCursor == -1)
		aCursor = mState.mCurrentCursor;
	...
	switch (aMode)
	{
	case ColorTextEdit::SelectionMode::Normal:
	case ColorTextEdit::SelectionMode::Word:
		break;
	case ColorTextEdit::SelectionMode::Line:
	{
...
void ColorTextEdit::SetTabSize(int aValue)
{
	mTabSize = std::max(0, std::min(32, aValue));
}

void ColorTextEdit::InsertText(const std::string& aValue, int aCursor)
{
	InsertText(aValue.c_str(), aCursor);
}

void ColorTextEdit::InsertText(const char* aValue, int aCursor)
{
	if (aValue == nullptr)
		return;
	if (aCursor == -1)
...
void ColorTextEdit::EnterText(const std::string& aValue)
{
	bool bRenderState = mWithinRender;
	mWithinRender = true; // 不设这个会导致EnsureCursorVisible里面位置计算逻辑出问题，让光标被反复置顶
	for (int i = 0; i < aValue.length(); i++)
	{
		EnterCharacter((ImWchar)aValue[i], false);
	}
	mWithinRender = bRenderState;
}

void ColorTextEdit::DeleteSelection(int aCursor)
{
	if (aCursor == -1)
		aCursor = mState.mCurrentCursor;
...
void ColorTextEdit::MoveUp(int aAmount, bool aSelect)
{
...
void ColorTextEdit::MoveDown(int aAmount, bool aSelect)
{
	if (HasSelection() && !aSelect)
	{
		for (int c = 0; c <= mState.mCurrentCursor; c++)
...
void ColorTextEdit::MoveLeft(int aAmount, bool aSelect, bool aWordMode)
{
	if (mLines.empty())
		return;
...
void ColorTextEdit::MoveRight(int aAmount, bool aSelect, bool aWordMode)
{
	if (mLines.empty())
		return;
...
void ColorTextEdit::MoveTop(bool aSelect)
{
	mState.mCurrentCursor = 0;
...
void ColorTextEdit::ColorTextEdit::MoveBottom(bool aSelect)
{
	mState.mCurrentCursor = 0;
...
void ColorTextEdit::MoveHome(bool aSelect)
{
	for (int c = 0; c <= mState.mCurrentCursor; c++)
	{
		auto oldPos = mState.mCursors[c].mCursorPosition;
...
void ColorTextEdit::MoveEnd(bool aSelect)
{
	for (int c = 0; c <= mState.mCurrentCursor; c++)
	{
		auto oldPos = mState.mCursors[c].mCursorPosition;
...
void ColorTextEdit::Delete(bool aWordMode)
{
	assert(!mReadOnly);
...
void ColorTextEdit::Backspace(bool aWordMode)
{
	assert(!mReadOnly);
...
void ColorTextEdit::SelectWordUnderCursor()
{
	auto c = GetCursorPosition();
	SetSelection(FindWordStart(c), FindWordEnd(c));
}

void ColorTextEdit::SelectAll()
{
	SetSelection(Coordinates(0, 0), Coordinates((int)mLines.size(), 0), SelectionMode::Line);
}

bool ColorTextEdit::HasSelection() const
{
	for (int c = 0; c <= mState.mCurrentCursor; c++)
...
bool ColorTextEdit::CanUndo() const
{
	return !mReadOnly && mUndoIndex > 0;
}

bool ColorTextEdit::CanRedo() const
{
	return !mReadOnly && mUndoIndex < (int)mUndoBuffer.size();
}

int ColorTextEdit::GetUndoCount() const
{
	return mUndoIndex;
}

int ColorTextEdit::GetRedoCount() const
{
	return (int)mUndoBuffer.size() - mUndoIndex;
}

void ColorTextEdit::Undo(int aSteps)
{
	while (CanUndo() && aSteps-- > 0)
		mUndoBuffer[--mUndoIndex].Undo(this);
}

void ColorTextEdit::Redo(int aSteps)
{
	while (CanRedo() && aSteps-- > 0)
		mUndoBuffer[mUndoIndex++].Redo(this);
}

const ColorTextEdit::Palette& ColorTextEdit::GetPyCharmDarkPalette()
{
	const static Palette p = { {
		...
		0x25a6a1ff,	// Number
		0x6aab73ff,	// String
		0xcf635aff, // Char literal
		0xffffffff, // Punctuation
		0x98ae4eff,	// Preprocessor
		0xcecdcbff, // Identifier
		0x93558cff, // Known identifier
		0x855389ff, // Preproc identifier
		0x7a7e85ff, // Comment (single line)
		0x4a8062ff, // Comment (multi line)
		0x1e1f22ff, // Background
		0xe0e0e0ff, // Cursor
		0x214283ff, // Selection
		0xff200080, // ErrorMarker
		0x43454aff, // BracketHighlighting
		0x90909090, // ControlCharacter
		0x0080f040, // Breakpoint
		0x464747ff, // Line number
		0x26282e40, // Current line fill
...
const ColorTextEdit::Palette& ColorTextEdit::GetDarkPalette()
{
	const static Palette p = { {
			0xb0b0b0ff,	// Default
			0x569cd6ff,	// Keyword
...
const ColorTextEdit::Palette& ColorTextEdit::GetMarianaPalette() // default
{
	const static Palette p = { {
			0xffffffff,	// Default
			0xc695c6ff,	// Keyword
...
const ColorTextEdit::Palette& ColorTextEdit::GetLightPalette()
{
	const static Palette p = { {
			0x404040ff,	// None
			0x060cffff,	// Keyword
...
const ColorTextEdit::Palette& ColorTextEdit::GetRetroBluePalette()
{
	const static Palette p = { {
			0xffff00ff,	// None
			0x00ffffff,	// Keyword
...
void ColorTextEdit::MergeCursorsIfPossible()
{
...
std::string ColorTextEdit::GetText() const
{
...
std::vector<std::string> ColorTextEdit::GetTextLines() const
{
...
std::string ColorTextEdit::GetClipboardText() const
{
	std::string result;
	for (int c = 0; c <= mState.mCurrentCursor; c++)
	{
...
std::string ColorTextEdit::GetSelectedText(int aCursor) const
{
	if (aCursor == -1)
		aCursor = mState.mCurrentCursor;
...
std::string ColorTextEdit::GetCurrentLineText()const
{
...
void ColorTextEdit::ProcessInputs()
{
}

void ColorTextEdit::Colorize(int aFromLine, int aLines)
{
...
void ColorTextEdit::ColorizeRange(int aFromLine, int aToLine)
{
...
void ColorTextEdit::ColorizeInternal()
{
...
float ColorTextEdit::TextDistanceToLineStart(const Coordinates& aFrom) const
{
...
void ColorTextEdit::EnsureCursorVisible(int aCursor)
{
	if (aCursor == -1)
		aCursor = mState.mCurrentCursor;
	...
	if (!mWithinRender)
	{
		...
		mScrollToCursor_CursorLineOnPage = aCursor;
		return;
	}
...
int ColorTextEdit::GetPageSize() const
{
	auto height = ImGui::GetWindowHeight() - 20.0f;
	return (int)floor(height / mCharAdvance.y);
}

ColorTextEdit::UndoRecord::UndoRecord(
	const std::vector<UndoOperation>& aOperations,
	ColorTextEdit::EditorState& aBefore,
	ColorTextEdit::EditorState& aAfter)
...
void ColorTextEdit::UndoRecord::Undo(ColorTextEdit* aEditor)
{
...
void ColorTextEdit::UndoRecord::Redo(ColorTextEdit* aEditor)
{
