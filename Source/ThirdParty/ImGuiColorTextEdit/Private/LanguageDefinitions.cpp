...
const ColorTextEdit::LanguageDefinition& ColorTextEdit::LanguageDefinition::Python()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
	{
		...
		static const char* const preprocIdentifier[] = {
			"self", "__init__", "kwargs", "args", "cls", "_", "__"
		};
		for (auto& ppi : preprocIdentifier)
		{
			Identifier id;
			id.mDeclaration = "Reserved identifier";
			langDef.mPreprocIdentifiers.insert(std::make_pair(std::string(ppi), id));
		}
		...
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("[a-zA-Z_][a-zA-Z0-9_]*", PaletteIndex::Identifier));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("[\\[\\]\\{\\}\\!\\@\\%\\^\\&\\*\\(\\)\\-\\+\\=\\~\\|\\<\\>\\?\\/\\;\\:\\,\\.]", PaletteIndex::Punctuation));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("__\\w+__", PaletteIndex::PreprocIdentifier));
